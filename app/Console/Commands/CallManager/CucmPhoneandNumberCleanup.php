...
class CucmPhoneandNumberCleanup extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($phones as $phone) {
            echo '##############################'.PHP_EOL;
            $count++;
            echo "{$count} of ".count($phones).' '.$phone['name'].PHP_EOL;
            ...
            if ($phone_reg_status != null) {
                echo "Skipping {$phone['name']} because it's status is: {$phone_reg_status}".PHP_EOL;
                continue;
            }
            ...
            foreach ($line_details as $line) {
                ...
                echo $number.': '.$line_description.PHP_EOL;

                if (isset($devices['device'])) {
                    if (is_array($devices['device'])) {
                        echo "Skipping {$number} as it has multiple devices assigned to it...".PHP_EOL;
                        print_r($devices);
                    } else {
                        print_r($devices);
                        $regex = "/{$SEARCH}/";
        ...
        foreach ($lines_to_delete as $line) {
            $line_count++;
            echo "Line: {$line_count} of ".count($lines_to_delete).": {$line['pattern']} | {$line['description']}".PHP_EOL;
        }
        echo '#############################################################'.PHP_EOL;
        foreach ($phones_to_delete as $phone) {
            $phone_count++;
            echo "Phone: {$phone_count} of ".count($phones_to_delete).": {$phone['name']} | {$phone['description']}".PHP_EOL;
        }
        echo '#############################################################'.PHP_EOL;
