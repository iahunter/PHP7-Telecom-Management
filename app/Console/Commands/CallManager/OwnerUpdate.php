...
class OwnerUpdate extends Command
{
    ...
    {
        echo 'Starting callmanager:owner-update... '.PHP_EOL;

        $users = $this->cucm->get_all_users();
        //print_r($users);
        echo 'Found '.count($users).' Users in CUCM'.PHP_EOL;

        // Do Stuff Here.
        echo 'Getting Sites... '.PHP_EOL;
        ...
        foreach ($SITES as $SITE) {
            echo '##########################################################################'.PHP_EOL;
            $SITE_COUNT++;
            echo 'Working On Site: '.$SITE_COUNT.'. '.$SITE.PHP_EOL;
            $phones = $this->cucm->list_all_phones_summary_by_site($SITE);
            //print_r($phones);
            echo 'Phone Count: '.count($phones).PHP_EOL;
            //die();
            if (count($phones) > 1) {
                $count = 0;
                $skipped = 0;
                            ...
                            if (count($description) > 1) {
                                ...
                                foreach ($users as $user) {
                                    if ((strtolower($first_name) == strtolower($user['firstName'])) && (strtolower($last_name) == strtolower($user['lastName']))) {
                                        echo '      We have a match!!! '.$phone['name'].' | '.$phone['description'].' | '.$first_name.' '.$last_name.PHP_EOL;
                ...
                echo "{$SITE_COUNT}. {$SITE} Results: ".PHP_EOL.'Skipped: '.$skipped.PHP_EOL.'Found: '.$found.PHP_EOL.'Unassigned: '.$unassigned.PHP_EOL.'Skipped Unassigned: '.$skipped_unassigned.PHP_EOL.'Removed Unassigned: '.$removed_unassigned.PHP_EOL.'Total: '.$count.PHP_EOL;
            }
        }
    }
}
