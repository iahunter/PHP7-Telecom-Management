    ...
    public function handle()
    {
		...
		foreach($sites as $site){
			...
			foreach($current_css as $key => $value){
				...
				print "Getting CSS Members...";
				print_r($css_details);

				
				if ($css_details['partitionUsage'] == "Intercom"){
					print "Skipping CSS {$value}... We don't want to act on Intercom CSSs";
					print_r($css_details);
					continue;
				}
				...
				if (empty($css_report)){
					print "Skipping CSS {$value}... CSS Report empty...";
					print_r($css_details);
					continue;
				}
				...
				foreach($newpartitions as $partition){
					...
					print "Match = ".$MATCH.PHP_EOL;
					if($MATCH == true){
						print "Skipping {$partition['name']} it already exists".PHP_EOL;
						continue;
					}
			...
			*/
			...
			if (in_array($RL911['name'],  $site_details['RouteList'])){
				print "Skipping {$RL911['name']}... Already exists...";
			}else{
				print "Adding {$RL911['name']} to CUCM...".PHP_EOL;
			...
			foreach($e911routes as $e911route){
				if (in_array($e911route['pattern'],  $site_details['RoutePattern'])){
					print "Skipping {$e911route['pattern']}... Already exists...";
				}else{
					
					print "Adding Pattern: {$e911route['pattern']} with {$e911route['routePartitionName']} to CUCM...".PHP_EOL;
					...
					if(is_array($member)){
						foreach ($member as $partition){
							...
							if (isset($partition['routePartitionName'])){
								...
								print $partition['routePartitionName']['_'];
							}else{
								return $RESULTS;
							}
