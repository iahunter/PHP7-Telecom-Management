...
class TeamsEgwSync extends Command
{
    ...
    public function handle()
    {
		...
		foreach($endpoints as $endpoint){
			...
			if($endpoint->extension && $endpoint->ip_pbx_name == $this->teamspbxname){
				...
				if(!isset($users[$endpoint->extension])){
					print "{$endpoint->extension} needs removed from the EGW...".PHP_EOL;
		...
		foreach($users as $number => $user_array){
			if(!in_array($number, $extensions)){
				print "Need to add {$number} to the EGW".PHP_EOL;
				$adds[] = $number; 
			}
		}
		
		$count = count($adds); 
		print "Found {$count} Endpoints to Add".PHP_EOL;
		print_r($adds);
		$this->add_extensions_to_egw($adds); 

		$count = count($deletes); 
		print "Found {$count} Endpoints to Delete".PHP_EOL;	
		print_r($deletes); 
	...
	private function add_extensions_to_egw($extensions){
		...
		foreach($extensions as $extension){
			...
			print "Adding Extension: {$extension}...".PHP_EOL;
			...
			print "Adding Extension: {$extension} Complete".PHP_EOL;
		}
	}
	...
	private function delete_extensions_from_egw($extensions){
		...
		foreach($extensions as $extension){
			...
			print "Deleting Extension: {$extension}...".PHP_EOL;
			...
			print "Deleting Extension: {$extension} Complete".PHP_EOL;
		}
	}
